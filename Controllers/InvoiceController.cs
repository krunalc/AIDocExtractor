...
namespace FileUploadReader.Controllers
{
  public class InvoiceController : Controller
  {
    ...
    public InvoiceController(IWebHostEnvironment environment, AzureCognitiveSettings azureSettings)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> UploadInvoice(IFormFile file, string sampleFilePath)
        {
            ...
            else if (!string.IsNullOrEmpty(sampleFilePath))
            {
                ...
                {
                    TempData["Error"] = $"Sample file not found: {sourcePath}";
                }
            }

            else
            {
                TempData["Error"] = "Please upload or select a sample invoice.";
            }
    ...
    public async Task<IActionResult> AnalyzeInvoice()
    {
      ...
      IInvoiceAdapter adapter = new SdkInvoiceAdapter(doc);
    ...
    [HttpPost]
    public async Task<IActionResult> AnalyzeInvoiceAPI()
    {
      var relativePath = HttpContext.Session.GetString("UploadedFilePath");
      var fullPath = Path.Combine(_environment.WebRootPath, relativePath.TrimStart('/'));
      ...
      var dataHelperResponse = await DataHelper.DataHelper.AnalyzeDocumentAsync<RootResponse>(fullPath, "prebuilt-invoice");
      ...
      Document doc = root?.analyzeResult.Documents.First();
      IInvoiceAdapter adapter = new ApiInvoiceAdapter(doc);
