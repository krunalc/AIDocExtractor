...
{
  public class ApiInvoiceAdapter : IInvoiceAdapter
  {
    private readonly ViewModels.Document _document;

    public ApiInvoiceAdapter(ViewModels.Document document)
    {
      _document = document;
    }
    ...
    public List<InvoiceField> ExtractFields()
    {
      ...
      if (_document.Fields.TryGetValue("Items", out var itemsField) && itemsField.Type == "array" &&
                                                  itemsField.ValueArray != null)
      {
        int count = 1;
        ...
        foreach (var item in itemsField.ValueArray)
        {
          ...
          {
            item.ValueObject.TryGetValue("Description", out var descField);
            item.ValueObject.TryGetValue("Quantity", out var qtyField);
            item.ValueObject.TryGetValue("UnitPrice", out var unitField);
            item.ValueObject.TryGetValue("Amount", out var amtField);

            string desc = descField?.Content ?? "N/A";
            string qty = qtyField?.Content ?? "N/A";
            string unit = unitField?.Content ?? "N/A";
            string amt = amtField?.Content ?? "N/A";

            // Calculate average confidence (or min/first)
