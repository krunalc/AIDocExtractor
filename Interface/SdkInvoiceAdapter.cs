...
public class SdkInvoiceAdapter : IInvoiceAdapter
{
    ...
    public List<InvoiceField> ExtractFields()
    {
        ...
        if (_document.Fields.TryGetValue("CustomerId", out var cusCode) &&
             cusCode != null)
        {
            fields.Add(new InvoiceField
            {
                Key = "Customer Code",
                Value = cusCode.Content,
                Confidence = cusCode.Confidence
            });
        }
        ...
        itemsField.FieldType == DocumentFieldType.List)
        {
            ...
            foreach (DocumentField itemField in itemsField.Value.AsList())
            {
                if (itemField.FieldType == DocumentFieldType.Dictionary)
                {
                    ...
                    string productCode = itemDict.TryGetValue("ProductCode", out var pField) ?
                        pField?.Content ?? "" : "N/A";
                    string tax = itemDict.TryGetValue("Tax", out var taxField) ?
                       taxField?.Content ?? "" : "N/A";
                    ...
                    List<float?> confidences = new()
            {
                ...
                amtField?.Confidence
                    ...
                    {
                        Key = $"Item {count}",
                        Value = $"Description: {desc}, Quantity: {qty}, Unit Price: {unit}, Amount: {amt}, Product Code: {productCode}, Tax: {tax}",
